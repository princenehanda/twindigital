...
function fleetCardHtml(t){
  ...
  return `<div class="card fleet-card card-link" onclick="window.location.href='truck.html?id=${t.id}'">
    <div class="fc-head">
...
function renderCustodySwitcher(current){
  ...
  if (!el) return;
  el.innerHTML = TRUCKS.map(t=>`<option value="${t.id}" ${t.id===current.id?'selected':''}>${t.id} \u2014 ${t.route.name}</option>`).join('');
  el.onchange = ()=>{ window.location.href = `custody.html?id=${el.value}`; };
}
...
function renderDepotLoadingBay(d){
  ...
  el.innerHTML = loading.length ? loading.map(t=>`
    <div class="loading-bay-row card-link" style="cursor:pointer;" onclick="window.location.href='truck.html?id=${t.id}'">
      <div>
        <strong style="font-size:13px;">${t.id}</strong>
        <div class="eyebrow" style="margin-top:2px;">${t.compartments.length} compartments \u00b7 ${fmtNum(t.loadedLoad)} / ${fmtNum(t.requestedLoad)} L</div>
      </div>
