...
function initRoleSwitcher(){
  ...
  function paint(){
    ...
    if (current === 'depot'){
      siteBar = `
        <select id="depot-site-picker" class="mono" style="background:var(--panel); border:1px solid var(--line-strong); color:var(--text); border-radius:999px; padding:6px 12px; font-size:12px; margin-left:8px;">
          ${DEPOTS.map(d=>`<option value="${d.id}" ${d.id===getMyDepotId()?'selected':''}>${d.name}</option>`).join('')}
        </select>`;
    } else if (current === 'ops'){
      const scope = getOpsScope();
      siteBar = `
        <select id="ops-scope-picker" class="mono" style="background:var(--panel); border:1px solid var(--line-strong); color:var(--text); border-radius:999px; padding:6px 12px; font-size:12px; margin-left:8px;">
          <option value="national" ${scope==='national'?'selected':''}>National (all regions)</option>
          ${getRegions().map(r=>`<option value="${r}" ${scope===r?'selected':''}>${r} region</option>`).join('')}
        </select>`;
    }
    mount.innerHTML = `
      <div style="display:flex; align-items:center;">
        <div class="role-switcher">
          ${Object.entries(ROLE_DEFS).map(([key,r])=>`
            <button type="button" class="role-btn ${key===current?'active':''}" data-role="${key}" title="${r.desc}">${r.short}</button>
          `).join('')}
        </div>
        ${siteBar}
      </div>`;
    ...
    const depotPicker = document.getElementById('depot-site-picker');
    if (depotPicker) depotPicker.addEventListener('change', ()=> setMyDepotId(depotPicker.value));
    const opsPicker = document.getElementById('ops-scope-picker');
    if (opsPicker) opsPicker.addEventListener('change', ()=> setOpsScope(opsPicker.value));
  }
  paint();
...
function renderKpis(){
  ...
  const alertsScoped = scopedDepots
    ? ALERTS.filter(a=>a.type!=='info' && scopedTrucks.some(t=>t.id===a.entity)).length
    : k.openAlerts;
  ...
  const items = [
    { label: scopeTag ? `Fuel in storage \u2014 ${scopeTag}` : 'Fuel in storage', val:fmtNum(storageVal/1000,0), unit:'kL', delta:`${storageCap? fmtNum(storageVal/storageCap*100,0):0}% of capacity`, cls:'flat', href: scopedDepots ? `depot.html?id=${scopedDepots[0].id}` : 'depot.html' },
    { label: scopeTag ? `Fuel in transit \u2014 ${scopeTag}` : 'Fuel in transit', val:fmtNum(transitVal/1000,1), unit:'kL', delta: scopedDepots ? `${scopedTrucks.length} tankers in region` : `${TRUCKS.length + 138} tankers loaded`, cls:'flat', href:'fleet.html', restrict: depotOnlyLock },
    ...
    { label: scopeTag ? `Fleet online \u2014 ${scopeTag}` : 'Fleet online', val:`${fleetOnlineScoped}/${fleetTotalScoped}`, unit:'', delta: scopedDepots ? 'in region' : `${k.fleetTotal-k.fleetOnline} offline / maintenance`, cls:'flat', href:'fleet.html', restrict: depotOnlyLock },
    { label:'Fuel loss (24h)', val:k.lossPct.toFixed(2), unit:'%', delta: scopedDepots ? 'fleet-wide, consolidated' : 'within 0.30% tolerance', cls:'up', href:'#loss-bar', restrict: depotOnlyLock },
    { label:'Revenue (24h)', val:'$'+fmtNum(k.revenue/1000,0), unit:'k', delta:'+3.1% week on week', cls:'up', restrict:'exec' },
    { label: scopeTag ? `Open alerts \u2014 ${scopeTag}` : 'Open alerts', val:alertsScoped, unit:'', delta: alertsScoped>3? 'needs attention':'nominal', cls:alertsScoped>3?'down':'flat', href:'#alerts-feed' },
  ];
  el.innerHTML = items.map(i=>`
    <div class="kpi ${i.href?'kpi-link':''}" ${i.href?`onclick="window.location.href='${i.href}'"`:''} ${i.restrict?`data-restrict="${i.restrict}"`:''}>
...
function initLeafletMap(){
  ...
  DEPOTS.forEach(d=>{
    ...
    m.on('click', ()=>{ window.location.href = `depot.html?id=${d.id}`; });
    depotMarkers[d.id] = m;
  });
  ...
  TRUCKS.forEach(t=>{
    ...
    m.on('click', ()=>{ window.location.href = `truck.html?id=${t.id}`; });
...
function renderTankfarmMini(){
  ...
  el.innerHTML = list.map(d=>`
    <div class="card card-link" style="padding:16px;" onclick="window.location.href='depot.html?id=${d.id}'">
...
function tankGaugeHtml(tk){
  ...
  const low = pct < 25;
  return `<div class="tank" style="flex:none;">
    <div class="tank-body">
...
function renderAlerts(targetId, limit){
  ...
  el.innerHTML = list.length ? list.map(a=>`
    <div class="alert-row card-link" style="cursor:pointer;" onclick="if(!event.target.closest('.alert-ack-btn')) window.location.href='truck.html?id=${a.entity}'">
...
function renderFleetSnapshot(){
  ...
  el.innerHTML = (rows.length ? `<table class="data-table">
    ...
    <tbody>${rows.map(t=>`
      <tr class="clickable" onclick="window.location.href='truck.html?id=${t.id}'">
...
function initFleet(){
  ...
  if (search) search.addEventListener('input', renderFleetTable);
  const filterStatus = document.getElementById('fleet-filter-status');
  if (filterStatus) filterStatus.addEventListener('change', renderFleetTable);
  LiveEngine.on(renderFleetTable);
  roleAwareRenderers.push(renderFleetTable);
}
...
function renderFleetTable(){
  ...
  tbody.innerHTML = rows.map(t=>{
    ...
    return `<tr class="clickable" onclick="window.location.href='truck.html?id=${t.id}'">
      <td><strong>${t.id}</strong><br><span class="mono" style="color:var(--text-faint); font-size:11px;">${t.reg}</span></td>
  ...
  }).join('') || `<tr><td colspan="9" style="text-align:center; color:var(--text-faint); padding:32px;">No vehicles match this filter.</td></tr>`;
}
...
function renderTruckSwitcher(current){
  ...
  el.addEventListener('change', ()=>{ window.location.href = `truck.html?id=${el.value}`; });
}
...
function initDepot(){
  ...
  LiveEngine.on(()=>{ renderDepotTanks(d); renderDepotReconciliation(d); });
  ...
  document.addEventListener('role-changed', ()=>{
    ...
    if (resolved.id !== d.id) window.location.href = `depot.html?id=${resolved.id}`;
  });
}
...
function renderDepotSwitcher(current){
  ...
  if (isDepot){
    ...
    el.onchange = ()=>{ window.location.href = `depot.html?id=${el.value}`; };
  }
}
...
function renderDepotPumps(d){
  ...
  const hint = roleAllowed('depot') ? '' :
    `<div class="action-lock-note" style="grid-column:1/-1;">\uD83D\uDD12 Pump control requires Depot Technician access \u2014 click a role above to preview.</div>`;
